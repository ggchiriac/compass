...
class SectionQueryTest:
    def __init__(self, course_guid):
        self.course_guid = course_guid

    def fetch_and_print_sections(self):
        # Fetch the first instructor for the course
        first_instructor = Instructor.objects.filter(section__course__guid=self.course_guid).order_by('id').first()

        if not first_instructor:
            print('No instructor found for this course.')
            return

        # Fetch sections only for the first instructor
        sections = (
            Section.objects.filter(course__guid=self.course_guid, instructor=first_instructor)
            .select_related('course', 'term', 'instructor')
            .prefetch_related(Prefetch('classmeeting_set', queryset=ClassMeeting.objects.order_by('meeting_number')))
        )

        for section in sections:
            self._print_section_info(section)

    def _print_section_info(self, section):
        print(f'Section ID: {section.id}')
        print(f'Section Name: {section.class_section}')
        print(f'Section instructor: {section.instructor}')
        ...
        for class_meeting in section.classmeeting_set.all():
            self._print_class_meeting_info(class_meeting)
        print()

    def _print_class_meeting_info(self, class_meeting):
...
if __name__ == '__main__':
    course_guid = '1252002334'
    test = SectionQueryTest(course_guid)
    test.fetch_and_print_sections()
