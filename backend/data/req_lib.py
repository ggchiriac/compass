    ...
    def _make_request(self, endpoint: str, **kwargs: Any) -> requests.Response:
        ...
        while attempts < max_attempts:
            try:
                ...
                print(
                    f"req_lib.py: Request timed out. Attempting again... ({attempts + 1}/{max_attempts})")
                attempts += 1
                if attempts == max_attempts:
                    print(
                        "req_lib.py: Maximum retry attempts reached. Request failed due to timeout.")
                    break
            except requests.exceptions.HTTPError as e:
                if e.response.status_code == 401 and attempts == 0:
                    self.configs._refreshToken(
